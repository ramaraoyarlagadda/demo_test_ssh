    ...
    If UCase(ProcessThisRow) = "Y" Or InStr(TestLabels, Environment("TestRunMode")) > 0 Or RUN_FROM_FX21_SECTION Then
        ...
        Select Case channelSource
            ...
            Case "FX21"
                ...
                ' IMPORTANT:
                ' keyDealUsingFX21 must be a Function that returns Boolean True/False.
                ' If it is a Sub, use: Call keyDealUsingFX21(TestStep)
                ' If it returns "PASS"/"FAIL", compare with UCase(dealKeyResult) = "PASS"
                ...
                If dealKeyResult = True Then
                    ...
                    Else
                        ' Non-STP path
                        ' NOTE: Replace hardcoded deal with DealNumber once confirmed for Non-STP flow
                        Call OpenCBSRecord("FX41", OracleFormWindow("FX41").OracleTextField("Deal No."), DealNumber)
                        SettlementFlag = OracleFormWindow("FX41").OracleTable("SettlementTable").GetFieldValue(1, "Settlement")
                        TestStep = TestStep + 1
                ...
                End If  ' keyDealUsingFX21 / dealKeyResult
...
'       Next        ' For RowNum
'
' 2) If keyDealUsingFX21 is a Sub (not Function), replace:
'       dealKeyResult = keyDealUsingFX21(TestStep)
'       If dealKeyResult = True Then
'    with:
'       Call keyDealUsingFX21(TestStep)
'       ' then continue without If, OR set dealKeyResult manually
'
' 3) If keyDealUsingFX21 returns "PASS"/"FAIL":
'       dealKeyResult = keyDealUsingFX21(TestStep)
'       If UCase(CStr(dealKeyResult)) = "PASS" Then
'
' 4) Required variables before FX21 amend section:
